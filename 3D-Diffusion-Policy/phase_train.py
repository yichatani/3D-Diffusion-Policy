...
class TrainDP3Workspace:
    ...
    def run(self):
        ...
        RUN_VALIDATION = True # False to reduce time cost
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                ...
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    obs_tensor = self.preprocess_observation(batch['obs'])
                    phase_latent = self.phase_encoder(obs_tensor)
                
                    # compute loss
                    t1_1 = time.time()
                    raw_loss, loss_dict = self.model.compute_loss(batch, phase_latent=phase_latent)
                ...
                with torch.no_grad():
                    ...
                    with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(tepoch):
                            ...
                            loss, loss_dict = self.model.compute_loss(batch,phase_latent=phase_latent)
            ...
            if (self.epoch % cfg.training.sample_every) == 0:
                with torch.no_grad():
                    ...
                    phase_latent = self.phase_encoder(obs_tensor)
                    result = policy.predict_action(obs_dict,phase_latent=phase_latent)
